...
function AddProductSiigo(props) {
  ...
  const grabarDatos = (datos) => {
    ...
    const leeProducto = async () => {
      params = {
        idproducto: "75abf5d7-5736-430d-aea5-1f78b9b8e7e8"
      };

      await axios({
        method: "post",
        url: "https://sitbusiness.co/cyclewear/api/712",
        params,
      })
        .then((res) => {
          newProd.push(res.data);
          console.log("NEW PRODUCTO : ", res.data);

          let params = {
            codigo: newProd[0].code,
            id: newProd[0].id,
            sku: newProd[0].reference,
            nombre: newProd[0].name,
            cantidad: 0,
            idgrupo: newProd[0].account_group.id,
            nombregrp: newProd[0].account_group.name,
            codigobarra: newProd[0].additional_fields.barcode,
            marca: newProd[0].additional_fields.brand,
            bodega: "Bodega tienda",
            nombrebodega: "Precio de venta 1",
            valor: newProd[0].prices[0].price_list[0].value,
            idiva: newProd[0].taxes[0].id,
            porcetajeiva: newProd[0].taxes[0].percentage,
            fechadecreacion: fechaactual,
            estado: 1
          }
          console.log("RESPUESTA PRODUCTO : ", params);

          const creaPrdBD = async () => {
            await axios({
              method: "post",
              ...
              .then((res) => {
                console.log("Grabando producto : ", params);

              })
              .catch(function (error) {
                console.log("Error Grabando producto");
              });
          };
          creaPrdBD();
        })
        .catch(function (error) {
          console.log("ERROR LEYENDO PRODUCTO");
        });
    };
    leeProducto();
